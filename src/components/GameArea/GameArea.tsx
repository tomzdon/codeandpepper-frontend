...
export const GameArea = () => {
    ...
    const handleFlip = useCallback(() => {
        getBattleResult({variables: {resourceType: gameMode}});
        setIsBouncing(true);
                ...
                {gameMode === ResourceType.PERSON && (
                    <>
                        <FlippedCard frontImage={Person} backImage={PersonBattle} onBounceEnd={handleBounceEnd}
                                     isBouncing={isBouncing} formattedJson={data?.randomEntityDuel?.player1}/>
                        <FlippedCard frontImage={Person} backImage={PersonBattle} onBounceEnd={handleBounceEnd}
                                     isBouncing={isBouncing} formattedJson={data?.randomEntityDuel?.player2}/>
                    </>
                )}
